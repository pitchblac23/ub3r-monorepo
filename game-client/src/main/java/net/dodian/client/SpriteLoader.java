...
public class SpriteLoader {
    ...
    public static boolean dumping = true;

    public static void createSprite(SpriteLoader sprite) {
		/*if (dumping) {
			File directory = new File(net.dodian.client.Signlink.findCacheDir() + "spriteDump/");
			if (!directory.exists()) {
				directory.mkdir();
			}
			net.dodian.client.DataUtils.writeFile(new File(directory.getAbsolutePath() + System.getProperty("file.separator") + sprite.id + ".png"), sprite.spriteData);
		}*/
     ...
     * @param sprite
     */
/* 	public static void createSprite(net.dodian.client.SpriteLoader sprite) {
		sprites[sprite.id] = new net.dodian.client.Sprite(sprite.spriteData);
		sprites[sprite.id].drawOffsetX = sprite.drawOffsetX;
		sprites[sprite.id].drawOffsetY = sprite.drawOffsetY;
	} */
