...
public class GameObjectData {
  ...
  public static void init() throws IOException {
    /*
     * System.out.println("Loading definitions..."); ArrayList<GameObjectData>
     * defs = (ArrayList<GameObjectData>) new XStream().fromXML(new
     * FileInputStream("data/content/objectDefinitions.xml")); definitions = new
     * GameObjectData[defs.size()]; for (int i = 0; i < defs.size(); i++) { //
     * Max items in the 377 cache.. // (Others only matter for newly // added
     * maps) GameObjectData def = defs.get(i); definitions[def.getId()] = def; }
     * System.out.println("Loaded " + defs.size() + " object definitions.");
     */
  }
   ...
   */
  public GameObjectData(int id, String name, String desc, int sizeX, int sizeY, boolean isSolid, boolean isWalkable,
      boolean hasActions, boolean unknown, int walkType) {
    this.id = id;
    this.name = name;
    if (name == null)
      this.name = "";
  ...
  public boolean setRangeAble() {
    int[] rangableObjects = { 14462, 14464, 14465, 14466, 14467, 14468, 14470, 14502, 11754, 3007, 980, 997, 4262,
        14437, 14438, 4437, 4439, 3487, 3457 };
    ...
    if (name != null) {
      ...
      String[] rangables = { "fungus", "mushroom", "sarcophagus", "counter", "plant", "altar", "pew", "log", "stump",
          "stool", "sign", "cart", "chest", "rock", "bush", "hedge", "chair", "table", "crate", "barrel", "box",
          "skeleton", "corpse", "vent", "stone", "rockslide" };
