...
import net.dodian.old.cache.util.ByteStream;
...
public class Region {
  ...
  public static void updateDoorClip(int x, int y, int height, int oldFace, int newFace) {
    /*
     * removeClipping(x, y, height); if (original == 0) { removeClipping(x - 1,
     * y, height); } else if (original == 1) { removeClipping(x, y + 1, height);
     * } else if (original == 2) { removeClipping(x + 1, y, height); } else if
     * (original == 3) { removeClipping(x, y - 1, height); }
     * addClippingForVariableObject(x, y, height, 0, face, false);
     */
  }
  ...
  public static void addObject(int objectId, int x, int y, int height, int direction, int type, boolean startUp) {
    ...
    if (objectId == 4439) {
      if (!startUp) {
        removeClippingForSolidObject(x, y, height, 2, 2, true);
      }
      return;
    }
  ...
  public static void removeObject(int id, int x, int y, int height, int direction, int type) {
    GameObjectData def = GameObjectData.forId(id);
    if (def == null) {
      return;
    }
    int xLength = def.getSizeX(direction);
    int yLength = def.getSizeY(direction);
    if (type == 22) {
      removeClipping(x, y, height, 0x200000);
    } else if (type >= 9) {
      removeClippingForSolidObject(x, y, height, xLength, yLength, def.unknown());
    } else if (type >= 0 && type <= 3) {
      removeClippingForVariableObject(x, y, height, type, direction, def.unknown());
    }
  }
  ...
  public static void load() {
    try {
      ...
      for (int i = 0; i < size; i++) {
        ...
        } catch (Exception e) { //Still missing regions!
