    ...
    public void ProcessPacket(Client client, int packetType, int packetSize) {
        ...
        if (itemId == 962 && player.freeSlots() > 0) {
            if (client.connectedFrom.equals(player.connectedFrom)) {
                client.sendMessage("Can't use it on another player from same address!");
                return;
            }
            ...
            if (1 + Misc.random(99) <= 50) {
                ...
                client.sendMessage("You got a " + client.GetItemName(partyHat).toLowerCase() + " from the cracker!");
            } else {
                player.addItem(partyHat, 1);
                player.sendMessage("You got a " + client.GetItemName(partyHat).toLowerCase() + " from " + client.getPlayerName());
                client.sendMessage(player.getPlayerName() + " got a  " + client.GetItemName(partyHat).toLowerCase() + " from the cracker!");
            }
        } else if (itemId == 962)
            client.sendMessage("Your partner need a slot free in their inventory!");
    }
}