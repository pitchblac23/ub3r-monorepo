...
fun startFishing(p: Client, clickObject: Int, clickOption: Int) {
        ...
        if (p.getLevel(Skills.FISHING) < Utils.fishReq[p.fishIndex]) {
            p.sendMessage("You need " + Utils.fishReq[p.fishIndex] + " fishing to fish here.")
            p.resetAction(true)
            return
        }
        if (!p.playerHasItem(Utils.fishTool[p.fishIndex])) {
            p.sendMessage("You need a " + p.GetItemName(Utils.fishTool[p.fishIndex]) + " to fish here.")
            p.resetAction(true)
            return
        }
    ...
    fun fish(p: Client) {
        ...
        if (!p.playerHasItem(314) && p.fishIndex == 1) {
            p.sendMessage("You do not have any feathers.")
            p.resetAction(true)
            return
        }
        if (p.fishIndex == 0) {
            ...
            if (p.getLevel(Skills.FISHING) >= 30 && random < 3) {
                p.addItem(331, 1)
                p.giveExperience(Utils.fishExp[p.fishIndex] + 100, Skills.FISHING)
                p.sendMessage("You catch some raw salmon.")
            } else {
                p.giveExperience(Utils.fishExp[p.fishIndex], Skills.FISHING)
                p.addItem(Utils.fishId[p.fishIndex], 1)
                p.sendMessage("You catch some raw trout.")
            }
        } else {
            p.giveExperience(Utils.fishExp[p.fishIndex], Skills.FISHING)
            p.addItem(Utils.fishId[p.fishIndex], 1)
            p.sendMessage("You catch some " + p.GetItemName(Utils.fishId[p.fishIndex]).lowercase(Locale.getDefault()) + ".")
        }
        p.requestAnim(Utils.fishAnim[p.fishIndex], 0)
