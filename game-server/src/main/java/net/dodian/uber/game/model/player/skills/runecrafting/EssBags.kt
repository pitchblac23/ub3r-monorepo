    ...
    companion object {
        ...
        fun fillEssencePouch(pouch: Int, c: Client): Boolean {
            ...
            if (slot in 0..3) {
                if (c.getLevel(Skills.RUNECRAFTING) < c.runePouchesLevel.get(slot)) {
                    c.send(SendMessage("You need level " + c.runePouchesLevel.get(slot) + " runecrafting to do this!"))
                    return true
                }
                if (c.runePouchesAmount.get(slot) >= c.runePouchesMaxAmount.get(slot)) {
                    c.send(SendMessage("This pouch is currently full of essence!"))
                    return true
                }
                ...
                } else c.send(SendMessage("No essence in your inventory!"))
                return true
            }
            return false
        }
        ...
        fun emptyEssencePouch(pouch: Int, c: Client): Boolean {
            ...
            if (slot in 0..3) {
                ...
                if (amount <= 0) {
                    c.send(SendMessage("Not enough inventory slot to empty the pouch!"))
                    return true
                }
                ...
                if (amount > 0) {
                    ...
                    c.runePouchesAmount[slot] -= amount
                } else c.send(SendMessage("No essence in your pouch!"))
                return true
            }
            return false
        }
